...
export class SpinnerComponent
  ...
  // Как понять, что спинер надо скрывать?
  ...
  // Нужно получить информацию, что спинер нужно скрыть
  ...
  public ngOnChanges(simpleChanges: SimpleChanges): void {
    if (
      simpleChanges.size &&
      simpleChanges.size.currentValue !== simpleChanges.size.previousValue
    ) {
  ...
  private draw(): void {
    const rotation =
      Math.round(
        ((new Date().getTime() - this.start.getTime()) / this.animationTime) *
          this.lines
      ) / this.lines;
    this.context.save();
    ...
    for (let i = 0; i < this.lines; i++) {
      ...
      this.context.fillStyle =
        'rgba(153, 153, 153,' + (i + 10) / this.lines + ')';
      this.context.fill();
